...
Can you include some updates:

1. In the summary tab show the numbers of different types of regcheck issues like how many: "sb vs rb coef/se mismatch", "sb vs rb SE mismatch" etc. Only show those issues types that occur at least once.

2. Make a new tybe "problems" that gives an overview of noted run problems. Below is some code in another package that aggregates the problems of a project and should give you an idea how they are stored for a project. That shall be a separate tab, not mixed with the issues tab. If the problems parcels exists, load it, otherwise check for single problem files.
