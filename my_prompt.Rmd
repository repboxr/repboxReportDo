...
Can you make new R files and function and possibly js and css files with prefix sr (summary report) that can do the following.

Given a set of project directories generate a joint report that  summarizes issues in projects. Ideally group by issues and put more severe on top (sb not replicated, the rb does not run, then coef diffs but sorted by number of coef diffs, just SE diffs can be ignored). Think of a good design, possibly add some interactivity and background info, at least the runid and codelines.

Describe first your design ideas then provide the code. Try to create fully separate files, no dependency on current repboxReportDo. Make paths such that all generated files are stored in an R package with directory:  

/home/rstudio/repbox/repboxReportSummary
